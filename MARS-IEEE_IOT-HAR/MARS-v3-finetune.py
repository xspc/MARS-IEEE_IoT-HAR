...
train_loader = dataf.DataLoader(dataset_train, batch_size=100, drop_last=True)  # 数据打乱在之前已经做了，为了方便，所以这样
test_loader = dataf.DataLoader(dataset_test, batch_size=100, drop_last=True)  # 因为最后一个batch会导致lstm的失效
...
class HARModel2(nn.Module):
    def __init__(self, n_hidden=96, n_layers=1, n_filters=32, stride=2, stride1D = 1, BATCH_SIZE = 100,
        ...
        self.fusion = fusion ### 1,2,3 mean feature level, decison level and decison level with fairness
...
def test(epoch):
    ...
    net.eval()
    val_accuracy = 0  # 到了验证模式，所以先清空accuracy
    val_losses = []
    ...
    with torch.no_grad():
        ...
        if best_acc < val_accuracy / test_involved:
            ...
            torch.save(net,
                       '/home/xspc/Downloads/IMUPose_Code/xspc_test/MARS_Code/MARS_v3_finetune_result.pkl')
        else:
            print("no best model,the best is : {:.6f}...".format(best_acc))

net = torch.load(
        '/home/xspc/Downloads/IMUPose_Code/xspc_test/proposed3/result/res_proposed3_3IMU/AMASS/network/proposed3_AMASS_model_3IMU.pkl')
net.netC = nn.Sequential(
    nn.Linear(96, 5),
    nn.ReLU(),
    nn.Linear(5, 5),
).cuda()
